...
cd /Volumes/Hera/Raw/BIDS/7TFF/rawlinks
...
link_glob(){
  ...
  if [ -d "$2" ]; then 
     ...
     [ "$expectn" != "$haven" ] && echo "[WARNING] rm $2 # to try again"
     return
  fi
...
if [ -z "$1" ]; then
  ...
  $scriptdir/000_gen_idlist.bash| while read d id sub_sess; do
    [ -d $bidsroot/$sub_sess ] && echo "# have $bidsroot/$sub_sess for raw $d" && continue
    [ -d $id ] && echo "# have rawlinks $id for raw $d" && continue
    link_subjalldcm $d $SKIP
  done
